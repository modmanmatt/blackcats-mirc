...
; desc: Web Server for mirc v7.14
; version: v6.6
...
menu menubar,status {
  ...
  ..$readL(n6):comeca
  ..$readL(n7):para
  ..$readL(n8):para | .timerrecom12 1 1 comeca
  .-
  $readL(n9)
  ..$readL(n10):{
...
alias -l init {
  ...
  if (($readf(autostart) == On) && ($event == start)) {
    $comeca(now) 
    checkupdate

    if ($readf(regserver) == Yes) {
      .timerregister 1 2 register_server
    }
  }
}
...
alias -l le {
  ...
  if ($hget(web_partial,%sock,&tmp)) {
    ...
    if (($bvar(&temp,0) > 2048) && ((!%i) || (%i > 2000))) { $ErrorHeader(%sock,414) }
    if (!%i) return
  }
  else {
    %i = $bfind(&temp, 1, 13 10 13 10)
  ...
  if ((POST* iswm %cmd) && ($getHeader(Content-Length:,&temp) < 5000)) {
    ; check if all post data has been received, if not a file post
    if ($calc($bvar(&temp,0) - %i - 3 - $getHeader(Content-Length:,&temp)) != 0) return
  }
  ...
  if ($bfind(&temp,1,.exe?/c+dir) || $bfind(&temp,1,?configdir) || $bfind(&temp,1,/phpMyAdmin) || $bfind(&temp,1,/e107_files/)) {
  ...
  if ((GET* iswm %cmd) || (HEAD* iswm %cmd) || (POST* iswm %cmd)) {
    ...
    if (GET / HTTP/1* iswm %cmd) {
      if ($hget(web_folders,0).item <= 1) {
        if ($test_access($hget(web_folders,1).item,%temp) != 200) { $ErrorHeader(%sock,401 Unauthorized) }
      ...
      if ($len(%file) > 1600) { %estado = 414 | %fixe = not | goto proximo }
      ...
      if ((robots.txt == %file) || (/robots.txt == %file)) { 
        var %path = $remove($shortfn($scriptdir) $+ configs\robots.txt,/)

        if ($isfile(%path)) { %fixe = %path }
        else { $ErrorHeader(%sock,404) }
        goto proximo
      }
      elseif ((%file == live) || (%file == /live)) {
      ...
      if (% isin %file) {
        ...
        %file = $replace($descodifica(%file,n),% $+ 20, $chr(32))
      }
      ...
      if ($test_access(%virt_dir,%temp) != 200) { $ErrorHeader(%sock,401 Unauthorized) }
      ...
      elseif ($isdir(%file_tmp)) {
        ...
        if (GET* iswm %cmd) {
          ...
          elseif ((%estado == 200) && (*.php* iswm $longfn(%fixe))) {
            if (!$isfile(%web_phpexe)) $ErrorHeader(%sock,416)
            else {
    ...
    $cabecalho(&temp,%fixe,%estado,%sock, %packsize)

    if (($bfind(&temp, 1,Icy-MetaData)) && (.mp3 isin %fixe)) {

      if (shoutcast_live isin $hget(web_socks,%sock)) {
        if (%web_shout-pos) sockmark %sock %web_shout-pos $+ $chr(1) $+ 0 $+ $chr(1) $+ real_time_sock_live $+ $chr(1) $+ %packsize
        else sockmark %sock 1 0 real_time_sock_live
      }
      else sockmark %sock $sock(%sock).mark $+ $chr(1) $+ real_time_sock
      halt
    }
    else {
...
alias -l phpExe {
  if (($isalias(FastCgi)) && (!$portfree(7070))) $FastCgi($1,$2,$3)

  var %resto = $remove($gettok($1,2,63),"), %init = $remove($gettok($1,1,63),"), %sock = $sockname

  if (((? isin $1) && (!%resto)) || (script> isin %resto) || ($len(%resto) > 190)) { $ErrorHeader(%sock,414) }
  ...
  if ($3 == p) {
    ...
    if ($bvar(&mine,0) < $remove($getHeader(Content-Length:,$2),")) halt
  ...
  .fwrite -n %post WshProcessEnv("GATEWAY_INTERFACE") = "CGI/1.1"
  ...
  var %content_type = $getHeader(Content-Type: , $2)
  if (%content_type) .fwrite -n %post WshProcessEnv("CONTENT_TYPE") = " $+ %content_type $+ "
  ; else .fwrite -n %post WshProcessEnv("CONTENT_TYPE") = "application/x-www-form-urlencoded"
  ...
  ;####### carrega headers como variaveis de ambiente!!!
  ...
  run -n %vbsfile
  .timerphpwait $+ %sock -om 0 10 { check %sock }
  halt
}

alias -l check {
  var %out = $tmp_dir $+ $1 $+ out.tmp
  var %post = $1 $+ sock

  if (!$isfile($tmp_dir $+ $1 $+ exe.vbs)) {
    .timerphpwait $+ $1 off

    ;makes new file without headers
    bread %out 1 $file(%out).size &fich
    var %i = $calc($bfind(&fich, 1, 13 10 13 10) + 4)
    bcopy &mine 1 &fich %i $bvar(&fich,0)
    ...
    var %enter, %ii = 1, %params, %cur

    while ($bnget(&fich,%ii,13) != -1) {
      %cur = $bnget(&fich,%ii,13)

      if (%enter) {
        if (Content-type: isin %cur) { %params = %params $+ $chr(13) $+ %cur | break }
        if (%params) %params = %params $+ $chr(13) $+ %cur
        else %params = $bnget(&fich,%ii,13)
      }
      if (Powered-By isin %cur) %enter = 1
      inc %ii
    }

    %params = $right(%params,$calc($len(%cur) - 1))

    if ($sock($1)) {
      var %k = $hget(web_partial,$1,&temp)

      if (Status: 302 Moved Temporarily isin $read(%out, 1))  {
        $cabecalho_php(&temp,%out,302,$1,%params)
        return
      }
      else {
        $cabecalho(&temp,%out,200,$1,,%params)
      }
    }
...
; $bnget(bin,ntok,delimiter) there are text size limitations
alias -l bnget {
  var %tmpz1,%tmpz2, %i = 1
...
alias -l initMhtml {
  ...
  if (%teste > 0) {
    ...
    if ($bfind(&temp,1,$hget(1 $+ $sockname,tag) $+ --)) {
      hadd -m web_socks $sockname $sockname Uploading File...
...
alias -l test_access {
  if ($gettok($hget(web_folders,$1),4,32) == Yes) {
    var %check = $wildtok($2,*Authorization: Basic *,1,13)

    if (%check) {
      var %data = $gettok(%check,3,32)
      var %user = $gettok($decode(%data,m),1,58)
      var %aut = $md5(%user $+ :Tontito Web Server Authenticate System: $+ $gettok($decode(%data,m),2,58))

      if (%aut == $gettok($hget(web_folders,$1),5,32)) { return 200 }
      elseif (%aut == $gettok($hget(web_passwd,%user),1,32) && $istok($hget(web_passwd,%user),$1,32)) { return 200 }
      else return 401
    }
    else {
      %check = $replace($wildtok($2,*Authorization: Digest *,1,13),$chr(44),$chr(32))

      var %user = $remove($gettok($wildtok(%check,username="*,1,32),2,61),")
      var %nonce = $remove($gettok($wildtok(%check,nonce="*,1,32),2,61),")
      var %response = $remove($gettok($wildtok(%check,response="*,1,32),2,61),")
      var %uri = $remove($gettok($wildtok(%check,uri="*,1,32),2,61),")
      var %opaque = $remove($gettok($wildtok(%check,opaque="*,1,32),2,61),")
      var %nc = $remove($gettok($gettok($wildtok(%check,nc=*,1,32),2,61),1,44),")
      var %cnonce = $remove($gettok($wildtok(%check,cnonce="*,1,32),2-,61),")
      var %compute = $remove($gettok($wildtok(%check,qop=*,1,32),2,61),")

      var %http = $gettok($2,1,32)

      if ($hget(web_accgranted,%nonce) != $sock($sockname).ip) return 401
      hadd -mu3600 web_accgranted %nonce $sock($sockname).ip
      var %auth = $gettok($hget(web_passwd,%user),1,32)

      if ((%compute == auth) || ((qop isin %check) && !%compute)) { %auth = $md5(%auth $+ : $+ %nonce $+ : $+ %nc $+ : $+ %cnonce $+ : $+ %compute $+ : $+ $md5(%http $+ : $+ %uri)) }
      elseif (%compute == auth-int) { %auth = $md5(%auth $+ : $+ %nonce $+ : $+ %nc $+ : $+ %cnonce $+ : $+ %compute $+ : $+ $md5(%http $+ : $+ %uri $+ : $+ $md5())) }    
      else { %auth = $md5(%auth $+ : $+ %nonce $+ : $+ $md5(%http $+ : $+ %uri)) }

      var %auth2 = $gettok($hget(web_folders,$1),5,32)
      if ((%compute == auth) || ((qop isin %check) && !%compute)) { %auth2 = $md5(%auth2 $+ : $+ %nonce $+ : $+ %nc $+ : $+ %cnonce $+ : $+ %compute $+ : $+ $md5(%http $+ : $+ %uri)) }
      elseif (%compute == auth-int) { %auth2 = $md5(%auth2 $+ : $+ %nonce $+ : $+ %nc $+ : $+ %cnonce $+ : $+ %compute $+ : $+ $md5(%http $+ : $+ %uri $+ : $+ $md5())) }
      else { %auth2 = $md5(%auth $+ : $+ %nonce $+ : $+ $md5(%http $+ : $+ %uri)) }

      if (%auth2 == %response) { return 200 }
      elseif (%auth == %response && $istok($hget(web_passwd,%user),$1,32)) { return 200 }
      else return 401
    }
  }
  return 200
...
alias -l envia {
  ...
  if ($sockerr) { $fecha(%sock,1) | return  }
  if ($sock(%sock).sq > 8192) { return }
  tokenize 1 $sock(%sock).mark

  if (!$4 || ($4 > 399) || ($5 <= 0)) { if ($5 <= 0) { return } | $fecha(%sock,1) }

  var %inc = 5
  if (real_time_sock isin $1-) { %inc = 4 }

  bread " $+ $1 $+ " $2 $calc(1024 * %inc) &fich
  sockwrite -n %sock &fich
  sockmark %sock $1 $+ $chr(1) $+ $calc($2 + (1024 * %inc)) $+ $chr(1) $+ $3 $+ $chr(1) $+ $4 $+ $chr(1) $+ $calc($5 - (1024 * %inc)) $+ $chr(1) $+ $6
  ...
  if ($timer(ww_bitrate)) inc %web_rate %inc

  if (($5 <= $calc(1024 * %inc)) || (%pos <= $calc(1024 * %inc))) {
    var %long = $longfn($1)
...
alias -l real_shout {
  if ($hget(%web_list,0).item == 0) $fecha($sockname,1)

  var %param = $sock($sockname).mark
  var %web_shout = $gettok(%param,1,1)
  var %web_shout_pos = $gettok(%param,2,1)
  set %web_shout-pos %web_shout
  var %packsize = $gettok(%param,4,1)
  if (%packsize !isnum) %packsize = 8196

  if (%web_shout <= 0 || ($sockname == $null)) { halt }

  if ($sock($sockname).sq <= 8192) {
    var %file = $shortfn($hget(%web_list,%web_shout))
    if ($isfile(%file) && ($file(%file).size > %web_shout_pos)) bread %file %web_shout_pos %packsize &data

    if ($bvar(&data,0) < %packsize) {
      inc %web_shout

      if (!$isfile($shortfn($hget(%web_list,%web_shout)))) { set %web_shout 1 }
      if (%web_shout > $hget(%web_list,0).item) { set %web_shout 1 }
      set %web_shout_pos $calc(%packsize - $bvar(&data,0))

      %file = $shortfn($hget(%web_list,%web_shout))
      if ($isfile(%file)) {
        bread %file 0 $calc(%packsize - $bvar(&data,0)) &fich_tmp
        bcopy -c &data $calc($bvar(&data,0) + 1) &fich_tmp 1 -1
      }
      else { sockclose $sockname | halt }
    }
    else { inc %web_shout_pos %packsize }

    if (%web_shout_pos <= %packsize) {
    ...
    if ($timer(ww_bitrate)) inc %web_rate $calc(%packsize / 1024)
    sockwrite $sockname &data
    sockwrite $sockname &metadata

    sockmark $sockname %web_shout $+ $chr(1) $+ %web_shout_pos $+ $chr(1) $+ $gettok(%param,3-,32)
  }
}
  ...
  if ($hget(web_socks,0).item > 0) {
    while (%i <= $hget(web_socks,0).item) {
      ...
      if (!$sock(%sock) || (($sock(%sock).rcvd == 0) && ($sock(%sock).ls > 60))) {
        hdel web_socks %sock
        echo Closed %sock with $sock(%sock).rcvd and $sock(%sock).ls
        sockclose %sock
      }
      else inc %i
    }
...
alias list_con {
  ...
  window -dlS @ListConnections 0 50 $window(-1).w 150
  while (%i <= $hget(web_socks,0).item) {
...
alias -l para {
  if ($sock(www)) {
    if ($isalias(start_upnp) && ($readf(Use_UPNP) == YES)) { $start_upnp(delete, $readf(port_list), TCP) }
...
alias -l comeca {
  ...
  if (!$sock(www)) {
    if ($version < 7) {
      ...
      if (!$portfree($readf(port_list),0.0.0.0)) { echo 4 -a $readL(n29) $readf(port_list) | halt }
    }

    ;only works for version 7.1
    if ($readf(Web_UPNP) == ON) {
      socklisten -p www $readf(port_list)
    }
    else {
  ...
  if ($version < 7.17) echo 3 Mirc Version 7.17 is highly recommended (due to bug fix and performance upgrades in sockets and hashtables)!
  if ($version > 7.17) echo 4 This Web server version hasn't been yet tested with Mirc $version $+ , some options may not work well.
...
alias -l regula {
  if ($window(@ListConnections)) titlebar @ListConnections - Total upload speed %web_rate kB/s
...
alias -l list {
  ...
  if ($4 == root) return
  var %time = $ticks
  var %i = 1, %j, %f, %dir = $1, %file = $tmp_dir $+ $sockname $+ index.tmp, %size
  var %shared = $2
  ...
  .fwrite %tmpfile <html><title> $readf(title) </title><meta HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=UTF-8"><body bgcolor=" $+ $readf(bgcolor) $+ " link=" $+ $readf(link) $+ " vlink=" $+ $readf(vlink) $+ " text=" $+ $readf(text) $+ "><div align="left">
...
alias -l make_folder_list {
  ...
  if (h !isin $file($1).attr) .fwrite -n $2 <tr><td><img src=" $+ /-_icons_-/folder.gif $+ "><a href=" $+ ./ $+ $3 $+ %tmp $+ / $+ "> %tmp1 </a></td><td></td><td></td></tr>
}
...
alias -l make_file_list {
  ...
  if ($left(%j,1) != /) { %j = / $+ %j }
  %size = $file(%f).size
  if (%size < 0) { %size = $calc(2^32 %size) }
  var %create = $asctime($file(%f).ctime, yyyy-mm-dd)

  if (%size < 10000000) { %size = $round($calc(%size / 1024),1) $+ Kb }
  else { %size = $round($calc(%size / 1048576),1) $+ Mb }

  if (*index.tmp !iswm %j) .fwrite -n $2 <tr><td><a href=" $+ / $+ $3 $+ %j $+ "> $nopath(%f) </a></td><td> %size </td><td> %create </td></tr>
}
...
alias -l list_sp {
  ...
  write -c $1 <html><meta HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=UTF-8"><title> $readf(title) </title><body bgcolor=" $+ $readf(bgcolor) $+ " link=" $+ $readf(link) $+ " vlink=" $+ $readf(vlink) $+ " text=" $+ $readf(text) $+ ">
  if ($isfile($readf(topimage))) { write -a $1 <p align="center"><img border="0" src=" /-_icons_-/top.jpg "></p> }

  if ($readf(showlive) == Yes) write -a $1 <p><img src=" $+ /-_icons_-/folder.gif $+ "><a href=" $+ / $+ live $+ / $+ "> live </a></p>
...
alias -l cabecalho {
  ...
  if ($bfind($1, 1,Icy-MetaData)) {
    sockmark $4 %fich $+ $chr(1) $+ %tem $+ $chr(1) $+ %tama $+ $chr(1) $+ %num $+ $chr(1) $+ %falta
    sockwrite -n $4 ICY %num
    if (%num < 400) {
      sockwrite -n $4 icy-notice1:<BR>This stream requires <a href="http://www.winamp.com/">Winamp or compatible</a><BR>
      sockwrite -n $4 icy-name:Tontito Web Sound %web_version
      sockwrite -n $4 icy-genre:All around
      sockwrite -n $4 icy-metaint: $+ $5
    ...
    if (%num < 400) {
      ...
      if (!%content) {
        ...
        elseif ((%ext isin css) || (Content-Type: text/css isin $6)) { sockwrite -n $4 Content-Type: text/css }
        ...
        elseif (%ext isin js) { sockwrite -n $4 Content-Type: text/javascript }
...
alias -l ErrorHeader {
  ...
  if ($sock($1).sq < 4000) {
    var %gmt = $gmt
    sockwrite -n $1 HTTP/1.0 $2 .
    sockwrite -n $1 Server: Tontito's Mirc Web Server %web_version
    sockwrite -n $1 Date: $asctime(%gmt,ddd $+ $chr(44) dd mmm yyyy HH:nn:ss) GMT

    if ($2 == 401 Unauthorized) { 
      var %nonce = $md5($ticks $+ $rand(1,99999) $+ $gmt $+ $sock($sockname).ip)
      hadd -mu60 web_accgranted %nonce $sock($sockname).ip

      if ($readf(web_auth_dig) == ON) sockwrite -n $1 WWW-Authenticate: Digest realm="Tontito Web Server Authenticate System", qop="auth", nonce=" $+ %nonce $+ ", opaque="5ccc069c403ebaf9f0171e9517f40e41"
      else sockwrite -n $1 WWW-Authenticate: Basic realm="Tontito Web Server Authenticate System" 
    }

    if ($2 == 302) { sockwrite -n $1 Location: $3 }
    else sockwrite -n $1 Content-Type: text/html
  ...
  if (($gettok($2,1,32) > 399) || ($2 == 302)) { $erro($gettok($2,1,32),$1,ok) }
}
...
alias -l erro {
  ...
  elseif ($1 == 503) %message = Service Unavailable for Mirc version < 6.32
...
alias -l fecha {
  ...
  ;request came from error

  if ($3) {
    .timerwaits $+ $1 -mo 0 200 waitToSend $1 | halt
  }

  if ($2 == 0) sclose $1
  .timerwait $+ $1 -mo 0 5000 sclose $1
  halt
}
...
alias -l sclose {
  if (!$event && ($sock($1).ls < $readf(connectiontimeout)) && ($readf(reuseconn) == Yes)) {
    hadd -m web_socks $1 $1 Waiting $sock($1).ls secs
  }
  elseif ($2 || !$sock($1) || (($sock($1).sq == 0) && ($sock($1).rq == 0))) {
    hdel web_socks $1
    hdel web_socks $mid($1,2,10)
    sockclose $1
    .timerwait $+ $1 off
...
alias -l sfecha {
  ...
  if ($hget(web_socks)) {
    hdel web_socks $sockname
    hdel web_socks 1 $+ $sockname
  }
  if ($hget(1 $+ $sockname)) { hfree 1 $+ $sockname }
...
dialog Web_version {
  ...
  size -1 -1 183 59
  option dbu
  button "Download", 1, 38 43 40 12
  button "Cancel", 2, 110 43 40 12, default cancel
  text "", 3, 10 3 168 12
  text "", 4, 10 19 169 21
}

on *:dialog:Web_version:sclick:*: {
  if ($did == 1) {
    run http://www.mircscripts.org/download.php?id=1898
  }
}
...
dialog web_conf {
  ...
  size -1 -1 213 241
  option dbu
  tab $readL(n37), 1, 1 0 207 217
  check $readL(n38), 2, 12 25 54 10, tab 1
  edit "", 5, 177 39 23 10, tab 1 limit 5
  text $readL(n40), 6, 125 40 52 8, tab 1
  text $readL(n41), 7, 11 53 51 8, tab 1
  edit "", 8, 61 52 16 10, tab 1 limit 3
  button $readL(n42), 9, 15 91 44 12, tab 1
  combo 10, 74 92 66 50, tab 1 sort size drop
  button $readL(n43), 11, 154 91 44 12, tab 1
  text "KB/s", 13, 78 53 18 8, tab 1
  check $readL(n44), 14, 13 109 89 10, tab 1
  check $readL(n45), 15, 119 109 83 10, tab 1
  box $readL(n46), 16, 7 72 198 79, tab 1
  box $readL(n47), 17, 7 154 198 53, tab 1
  combo 18, 74 190 67 50, tab 1 sort size drop
  button $readL(n48), 19, 15 190 44 12, tab 1
  button $readL(n49), 20, 154 190 44 12, tab 1
  check $readL(n50), 21, 12 162 96 10, tab 1
  edit "", 22, 36 175 164 10, tab 1 autohs
  text $readL(n51), 23, 9 177 27 8, tab 1
  edit "", 24, 161 163 21 10, tab 1
  text $readL(n52), 25, 117 164 44 8, tab 1
  text $readL(n53), 26, 182 164 20 8, tab 1
  box "", 27, 7 16 198 53, tab 1
  text "", 50, 67 27 135 8, tab 1
  check $readL(n54), 51, 13 122 63 10, tab 1
  edit "", 52, 37 136 50 10, tab 1
  edit "", 53, 143 136 53 10, tab 1 pass
  text $readL(n55), 54, 105 137 38 8, tab 1
  text $readL(n56), 55, 13 137 23 8, tab 1
  text "", 56, 81 126 116 8, tab 1
  link "", 12, 11 80 191 8, tab 1
  check $readL(n127), 110, 12 38 70 10, tab 1
  tab $readL(n57), 28
  box $readL(n58), 29, 8 20 196 43, tab 28
  ...
  box $readL(n62), 34, 8 67 196 43, tab 28
  check $readL(n63), 35, 15 76 59 10, tab 28
  check $readL(n64), 36, 142 76 58 10, tab 28
  combo 37, 72 91 70 50, tab 28 sort size drop
  button $readL(n60), 38, 16 90 44 12, tab 28
  button $readL(n61), 39, 153 90 44 12, tab 28
  box $readL(n65), 71, 8 114 196 35, tab 28
  button $readL(n60), 73, 16 127 44 12, tab 28
  combo 74, 72 128 70 50, tab 28 size drop
  button $readL(n61), 75, 153 127 44 12, tab 28
  box $readL(n129), 108, 8 154 196 31, tab 28
  check $readL(n128), 109, 16 165 67 10, tab 28
  tab $readL(n101), 40
  box $readl(n82), 41, 7 16 198 83, tab 40
  text $readL(n83), 42, 13 43 64 8, tab 40
  text $readL(n84), 43, 13 59 64 8, tab 40
  edit "", 44, 77 42 16 10, tab 40 limit 3
  edit "", 45, 77 58 16 10, tab 40 limit 3
  link $readL(n85), 49, 116 59 86 8, tab 40
  ...
  check $readL(n125), 105, 13 27 70 10, tab 40
  text $readL(n126), 107, 99 28 55 8, tab 40
  edit "", 106, 154 27 19 10, tab 40
  text "sec", 104, 174 28 15 8, tab 40
  tab $readL(n86), 57
  ...
  tab $readL(n118), 72
  button $readL(n95), 76, 157 36 41 12, tab 72
  box $readL(n94), 77, 6 16 199 63, tab 72
  edit "", 78, 80 61 57 10, tab 72
  button $readL(n95), 79, 157 60 41 12, tab 72
  text $readL(n96), 80, 94 50 33 8, tab 72
  text $readL(n97), 81, 15 38 137 8, tab 72
  text $readL(n98), 82, 15 62 64 8, tab 72
  check $readL(n130), 96, 11 24 50 10, tab 72
  box $readL(n103), 83, 6 84 199 39, tab 72
  check $readL(n102), 84, 11 93 157 10, tab 72
  text "", 85, 11 108 136 8, tab 72
  button $readL(n104), 86, 147 106 56 12, tab 72
  box $readL(n119), 97, 6 128 199 59, tab 72
  check $readL(n122), 103, 13 136 50 10, tab 72
  button $readL(n121), 101, 14 150 44 12, tab 72
  button $readL(n123), 112, 14 171 44 12, tab 72
  button $readL(n120), 99, 154 150 44 12, tab 72
  button $readL(n124), 113, 154 170 44 12, tab 72
  combo 100, 64 138 85 50, tab 72 size drop
  link "", 102, 67 163 78 8, tab 72
  button $readL(n67), 46, 19 224 40 12, ok
  button $readL(n68), 47, 152 224 40 12, cancel
  button $readL(n69), 48, 88 224 37 12
}
...
on *:dialog:web_conf:sclick:*: {
  ...
  if (($did == 51) && ($did(web_conf,10))) {
    var %check = No, %login
    if ($did(web_conf,52) && $did(web_conf,53)) { 
    %login = $md5($did(web_conf,52) $+ :Tontito Web Server Authenticate System: $+ $did(web_conf,53)) }
  ...
  if ($did == 84) {
    if ($did(web_conf,84).state == 1) {
      ...
      if (!%web_phpexe) {
        ...
        if (!$isfile($deltok(%web_phpexe,-1,92) $+ \php.ini)) {
          write -c $deltok(%web_phpexe,-1,92) $+ \php.ini session.save_path = $longfn($tmp_dir)
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini extension_dir =" $+ $deltok(%web_phpexe,-1,92) $+ \ext"

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini [PHP_GD2]
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini extension=php_gd2.dll

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini [PHP_IMAP]
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini extension=php_imap.dll

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini [PHP_MYSQL]
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini extension=php_mysql.dll

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini [PHP_MYSQLI]
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini extension=php_mysqli.dll

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini variables_order = "GPCS"

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini post_max_size = 8M
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini file_uploads = On
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini upload_max_filesize = 2M
          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini upload_tmp_dir = $longfn($tmp_dir)

          write -a $deltok(%web_phpexe,-1,92) $+ \php.ini date.timezone =Etc/GMT
        }
  ...
  if ($did == 90) {
    ...
    %pass = $md5(%user $+ :Tontito Web Server Authenticate System: $+ %pass)
  ...
  if ($did == 89) {
    if (!$istok($hget(web_passwd,$did(web_conf,95)),$did(web_conf,98).seltext,32)) {
  ...
  if ($did == 95) {
    ...
    var %i = 1
    while (%i <= $hget(web_folders,0).item) {
      if (!$istok($hget(web_passwd,%folder),$hget(web_folders,%i).item,32)) did -a web_conf 98 $hget(web_folders,%i).item
      inc %i
    }
  }
  ...
  if ($did == 96) {
    var %check = Yes
    if ($did(web_conf,96).state != 1) %check = No
    $writef(showlive,%check)
  }

  if ($did == 99) {
    var %domain = $$?="Please type your ring domain"
    var %pass = $$?*="Please type your ring password"
    $writef(ringkey,%pass)
    $writef(ringdomain,%domain)
  }

  if (($did == 100) && $readf(ringkey)) {
    did -a web_conf 102 http:// $+ $gettok($did(web_conf,100),1,32)
  }

  if (($did == 101) && $readf(ringkey)) {
    register_server
    did -a web_conf 102 Registering server on central...
  }

  if ($did == 102) {
    if (!$did(web_conf,100)) { did -a web_conf 102 Select an item in the combo! | return }

    var %item = $hfind(web_servers,* $+ $gettok($did(web_conf,100),1,32) $+ *,1,w).item
    if (!%item) { did -a web_conf 102 Select an item in the combo! | return }
  ...
  if ($did == 103) {
    var %check = Yes
    if ($did(web_conf,103).state != 1) { 
      did -b web_conf 99,100,101,102,112,113
      %check = No 
    }
    else {
      did -e web_conf 99,100,101,102,112,113
    }

    $writef(regserver,%check)
  }


  if ($did == 105) {
    var %check = Yes, %timeout = 60
    if ($did(web_conf,106)) %timeout = $did(web_conf,106)

    if ($did(web_conf,105).state != 1) { 
      did -b web_conf 106,107
      %check = No 
    }
    else {
      did -e web_conf 106,107
    }

    $writef(reuseconn,%check)
    $writef(connectiontimeout, %timeout)
  }


  if ($did == 109) {
    if ($did(web_conf,109).state == 1) $writef(web_auth_dig,ON)
    else $writef(web_auth_dig,OFF)
  }


  if ($did == 110) {
    if ($did(web_conf,110).state == 1) $writef(web_UPNP,ON)
    else $writef(web_UPNP,OFF)
  }


  if ($did == 112) {
    if (!$readf(ringkey) || !$readf(ringdomain)) {
      did -a web_conf 102 Please set your details first!
      return
    }

    var %name = $$?="Please type ring name"
    var %pass = $$?*="Please type ring password"
...
alias -l fill_conf {
  ...
  if ($readf(showlive) == Yes) { did -c web_conf 96 }

  if ($readf(phpexe)) { did -c web_conf 84 | did -a web_conf 85 $readf(phpexe) }
  else { did -b web_conf 85,86 | did -a web_conf 85 Not set }

  if ($readf(regserver) == Yes) did -c web_conf 103

  if ($readf(reuseconn) == Yes) did -c web_conf 105
  if ($readf(connectiontimeout)) did -a web_conf 106 $readf(connectiontimeout)
  else did -a web_conf 106 60

  if ($readf(web_auth_dig) == ON) did -c web_conf 109

  if ($readf(web_UPNP) == ON) did -c web_conf 110
...
alias -l load_comb7 {
  did -r web_conf 100
  var %i = 1
  while (%i <= $hget(web_servers,0).item) {
    did -a web_conf 100 $hget(web_servers,%i).item Ring( $+ $hget(web_servers,%i).data $+ ) | inc %i
  }
}
...
alias leave_ring {
  var %tick = $ticks
  sockopen update $+ %tick tontito.no-ip.com 80
  sockmark update $+ %tick leave $1 $2
}

alias join_ring {
  var %tick = $ticks
  sockopen update $+ %tick tontito.no-ip.com 80
  sockmark update $+ %tick join $1 $2
}

alias register_server {
  var %tick = $ticks
  sockopen update $+ %tick tontito.no-ip.com 80
  sockmark update $+ %tick register
}

alias -l checkupdate {
  ; never add http:// to a connecting socket...
  sockopen update $+ $ticks tontito.no-ip.com 80
}
...
on *:sockopen:update*: {
  if ($hget(web_servers)) hfree web_servers

  if ($sockerr) { echo -a $readL(n113) $sock($sockname).wsmsg | return }
  var %mark = $sock($sockname).mark

  if (!%mark) {
    sockwrite -n $sockname GET versions/versions.txt HTTP/1.1
    sockwrite -n $sockname Host: tontito.no-ip.com
    sockwrite -n $sockname User-Agent: Web Server %web_version on $version and $os
    sockwrite -n $sockname Pragma: no-cache
    sockwrite -n $sockname Cache-Control: no-cache
    sockwrite -n $sockname
  }
  elseif (%mark == register) {
    var %server_name = $readf(ringdomain)
    var %key = $readf(ringkey)

    sockwrite -n $sockname GET versions/register/ring.mhtml?id= $+ $md5(%key) $+ &alias= $+ %server_name $+ : $+ $readf(port_list) HTTP/1.1
    sockwrite -n $sockname Host: tontito.no-ip.com
    sockwrite -n $sockname User-Agent: Web Server %web_version on $version and $os
    sockwrite -n $sockname Pragma: no-cache
    sockwrite -n $sockname Cache-Control: no-cache
  ...
  elseif ($gettok(%mark,1,32) == join || $gettok(%mark,1,32) == leave) {
    var %server_name = $readf(ringdomain)
    var %key = $readf(ringkey)

    sockwrite -n $sockname GET versions/register/ring.mhtml?id= $+ $md5(%key) $+ &alias= $+ %server_name $+ : $+ $readf(port_list) $+ &action= $+ $gettok(%mark,1,32) $+ &ring= $+ $gettok(%mark,2,32) $+ &pass= $+ $gettok(%mark,3,32) HTTP/1.1
    sockwrite -n $sockname Host: tontito.no-ip.com
    sockwrite -n $sockname User-Agent: Web Server %web_version on $version and $os
    sockwrite -n $sockname Pragma: no-cache
    sockwrite -n $sockname Cache-Control: no-cache
...
on *:sockread:update*: {
  ...
  if (!%mark) {
    if ((version isin %i) && ($chr(35) !isin %i) && (:web: isin %i)) {
      %version = $gettok(%i,3,58)
      %version = $gettok(%version,1-2,46) $+ $gettok(%version,3,46)

      var %my_version = $gettok(%web_version,1,32)
      ...
      elseif (%get_where == mircscripts) %get_where = There is a new Web Server version %version at http://www.mircscripts.org/download.php?id=1898
      else %get_where = at http://tontito.no-ip.com/versions/ $+ %get_where Plz report bugs to tontitoscript@gmail.com

      if ((%version isnum) && (%version > %my_version)) {
        if (!$dialog(web_version)) dialog -md web_version web_version
        did -a web_version 3 %get_where
        did -a web_version 4 Release Notes: %rnotes
        return
      }
      elseif (beta isin %version) { 
        echo 7 -a There is a recent (beta) version %version $+  %get_where
        if (%rnotes) { echo -a 3,7 Release Notes: %rnotes }
      }
      elseif (RC isincs %version) {
        echo 7 -a There is a (Release Candidate) version %version $+  %get_where
        if (%rnotes) { echo -a 3,7 Release Notes: %rnotes }
      }
      else {
        echo -a 3 $readL(n115)
    ...
    if (<br> isin %i) {
      hadd -m web_servers $remove(%i,<br>,</br>)
    }
  }

  if ($dialog(web_conf)) {
    load_comb7
    did -a web_conf 102 Select item from combo
  }
}
;###
